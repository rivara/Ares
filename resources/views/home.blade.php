...
<script>
    $.ajaxSetup({
        ...
        $('#example').DataTable({
            ...
            columnDefs: [
                {
                    targets: [1], // Índice de la columna, comienza desde 0
                ...
                {
                    targets: [2], // Índice de la columna, comienza desde 0
                    render: function(data, type, row, meta) {
                        if (type === 'display') {
                ...
                {
                    targets: [3], // Índice de la columna, comienza desde 0
                    render: function(data, type, row, meta) {
                        if (type === 'display') {
          ...
          render : function(data, type, full, meta){
            ...
            if (type === 'display') {
                    return '<form action="" method="get"><input type="hidden" name="id" value="'+full.id+'"><input type="hidden" name="name" value="'+full.id+'"><input type="submit"  style="margin-bottom: 10px;" class="btn btn-danger pull-right"  /> <i class="fa fa-minus"></i></input></form>';
            }
          },
          targets :4
        },
    ...
    $(document).ready(function() {
            ...
            columnDefs: [
                {
                    ...
                    render: function(data, type, row, meta) {
                        if (type === 'display') {
                            return '<div contenteditable class="editable-column">' + data + '</div>';
                        }
                        return data;
                    }
                }
        ...
        $('#example').on('blur', '.editable-column', function() {
           
            var newValue = $(this).text();
            var rowIndex = $(this).closest('tr').index();
            // Llamada AJAX para guardar el nuevo valor en la base de datos
            // utilizando el ID del elemento de la fila y el nombre de la columna
            $.ajax({
            type: 'post',
            url:"{{ route('update') }}",
            dataType: 'json', // El tipo de datos que esperas recibir
